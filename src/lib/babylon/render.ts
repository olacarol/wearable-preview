...
import { fetchTokenURI } from '../../components/Colletions/fetchTokenURI'; // adicionado 
/**
...
export async function render(canvas: HTMLCanvasElement, config: PreviewConfig): Promise<IPreviewController> {
  ...
  // Chama a função para buscar o tokenURI //adicionado
  const tokenURI = await fetchTokenURI(); // Isso é assíncrono, então aguarde o resultado.
  console.log(tokenURI); // Verifique o tokenURI no console //adicionado
